...
describe('app update', () => {
  ...
  it('still says a release is out when it can’t update itself', () => {
    render(<AppUpdateSection st={st({ state: 'unsupported', target: '0.2.8', why: 'drag it to Applications first' })} />)
    expect(screen.getByText(/RiftRoute 0.2.8 is out/)).toHaveTextContent('drag it to Applications first')
  })
